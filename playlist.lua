...
function mopidy_player()
    ...
    local json_response = read_CLI("curl -d '[" .. URL.currentUrl ..
                                       URL.trackListUrl .. URL.TimeUrl ..
                                       URL.indexUrl .. URL.totalUrl ..
    ...
    for N in pairs(trackList) do
        ...
        if trackList[N].tlid >= current.tlid and trackList[N].track.length ~=
            nil then
        ...
        if (trackList[N].tlid >= current.tlid and trackList[N].tlid <
            current.tlid + show_tracks) or
            (total - index < show_tracks and N > total - show_tracks) then
    ...
    display_image({
        coord = {x = 5, y = 633},
        img = "/run/user/1000/album_cover.png"
    })
    text_by_center({x = 23, y = 692}, index .. "/" .. total,
                   {background = {color = "0x000000", alpha = 0.5}})
    text_by_center({x = 23, y = 709}, totalTime, {})
    draw_album_progress_line({x_start = 5, x_end = 313, y = 626}, {
        total = album_track_count,
        current = current.track.track_no,
        pass = time / current.track.length
...
-----------------
function mpv_player()
    local position = read_CLI(
                         'echo \'{ "command": ["get_property", "time-pos"] }\' | socat - /run/user/1000/mpvsocket | jq .data | tr -d \'"\' | cut -d\'.\' -f 1')
    if position ~= "" and string.find(position, "Connection refused") == nil then
        local duration = read_CLI(
                             'echo \'{ "command": ["get_property", "duration"] }\' | socat - /run/user/1000/mpvsocket | jq .data | tr -d \'"\' | cut -d\'.\' -f 1')
        local remaining = read_CLI(
                              'echo \'{ "command": ["get_property", "time-remaining"] }\' | socat - /run/user/1000/mpvsocket | jq .data | tr -d \'"\' | cut -d\'.\' -f 1')
        local title = read_CLI(
                          'echo \'{ "command": ["get_property", "media-title"] }\' | socat - /run/user/1000/mpvsocket | jq .data | tr -d \'"\'')

        return draw_player(scripts .. "img/mpv.png", "new_gradient", title, nil,
                           duration, position, time_format(remaining * 1000))
    end
...
--- Отрсиовка плеера ---
------------------------
function draw_player(icn, clr, title, file, total_time, playing_time, el_time,
                     artist, mediaSrc, img)
...
function get_img(mediaSrc, img)
    ...
    if file_exists(path) == false then
        local img_tml_command =
            'ffmpeg -loglevel 0 -y -i %s -pix_fmt rgba -vf "scale=45:-1" "%s"'
